describe('TALLER Y GASTOS - GASTOS GENERALES - Validación completa con gestión de errores y reporte a Excel', () => {
    ...
    const casos = [
        ...
        { numero: 5, nombre: 'TC005 - Filtrar por Inicio (fecha)', funcion: () => ejecutarFiltroIndividual(5), prioridad: 'ALTA' },
        { numero: 6, nombre: 'TC006 - Filtrar por Código', funcion: () => ejecutarFiltroIndividual(6), prioridad: 'ALTA' },
        { numero: 7, nombre: 'TC007 - Filtrar por Fin (fecha)', funcion: () => ejecutarFiltroIndividual(7), prioridad: 'ALTA' },
        { numero: 8, nombre: 'TC008 - Filtrar por Importe', funcion: () => ejecutarFiltroIndividual(8), prioridad: 'ALTA' },
        { numero: 9, nombre: 'TC009 - Filtrar por Descripción (contenga)', funcion: () => ejecutarFiltroIndividual(9), prioridad: 'ALTA' },
        { numero: 10, nombre: 'TC010 - Filtrar por Factura', funcion: () => ejecutarFiltroIndividual(10), prioridad: 'MEDIA' },
        { numero: 11, nombre: 'TC011 - Filtrar por Nómina', funcion: () => ejecutarFiltroIndividual(11), prioridad: 'MEDIA' },
        ...
        { numero: 15, nombre: 'TC015 - Búsqueda con espacios inicio/fin', funcion: () => ejecutarFiltroIndividual(15), prioridad: 'MEDIA' },
    ...
    function ejecutarFiltroIndividual(numeroCaso) {
        ...
        // Obtener datos del Excel para TallerYGastos-Gastos Generales
        return cy.obtenerDatosExcel('TallerYGastos-Gastos Generales').then((datosFiltros) => {
            ...
            if (filtroEspecifico.valor_etiqueta_1 === 'columna') {
                ...
                cy.get('select[name="column"], select#column').should('be.visible').then($select => {
                    ...
                    switch(filtroEspecifico.dato_1) {
                        case 'Inicio':
                            columnaEncontrada = options.find(opt => opt.includes('Inicio') || opt.includes('Start'));
                            break;
                        case 'Código':
                            columnaEncontrada = options.find(opt => opt.includes('Código') || opt.includes('Code'));
                            break;
                        case 'Fin':
                            columnaEncontrada = options.find(opt => opt.includes('Fin') || opt.includes('End'));
                            break;
                        case 'Importe':
                        ...
                        case 'Factura':
                            ...
                            break;
                        case 'Nómina':
                            columnaEncontrada = options.find(opt => opt.includes('Nómina') || opt.includes('Payroll'));
                            break;
                        default:
                ...
                if (!filtroEspecifico.dato_2 || filtroEspecifico.dato_2.trim() === '') {
                    ...
                    cy.registrarResultados({
                        numero: numeroCaso,
                        nombre: `TC${numeroCasoFormateado} - Filtrar gastos generales por ${filtroEspecifico.dato_1}`,
                ...
                cy.get('body').then($body => {
                    ...
                    if (busquedaSeAplico) {
                        ...
                        cy.registrarResultados({
                            numero: numeroCaso,
                            nombre: `TC${numeroCasoFormateado} - Búsqueda general de gastos generales`,
