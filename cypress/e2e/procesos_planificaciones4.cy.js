// procesos_planificaciones4.cy.js
describe('PROCESOS - PLANIFICACIÓN - Casos TC046-TC061', () => {
  ...
  after(() => {
    cy.log('Procesando resultados finales para Procesos (Planificación) - Casos 46-61');
    cy.procesarResultadosPantalla('Procesos (Planificación)');
  });

  it('Ejecutar casos TC046-TC061 desde Excel', () => {
    cy.obtenerDatosExcel('PROCESOS-PLANIFICACION').then((casos) => {
      ...
      casosPlanificacion.forEach((caso, index) => {
        ...
        let funcion;
        // Mapeo específico para TC046-TC061 (casos avanzados)
        if (numero === 46) funcion = eliminarConSeleccion;
