// procesos_planificaciones3.cy.js
describe('PROCESOS - PLANIFICACIÓN - Casos TC031-TC045', () => {
  ...
  after(() => {
    cy.log('Procesando resultados finales para Procesos (Planificación) - Casos 31-45');
    cy.procesarResultadosPantalla('Procesos (Planificación)');
  });

  it('Ejecutar casos TC031-TC045 desde Excel', () => {
    cy.obtenerDatosExcel('PROCESOS-PLANIFICACION').then((casos) => {
      ...
      cy.log(`Casos filtrados para Planificación (31-45): ${casosPlanificacion.length}`);
      ...
      const ejecutarCaso = (index) => {
        ...
        let funcion;
        // Mapeo específico para TC031-TC045 (búsquedas generales y filtros)
        if (numero >= 31 && numero <= 40) funcion = () => ejecutarFiltroIndividual(numero);
          ...
          cy.log(`⚠️ Caso ${numero} no está en el rango TC031-TC045 - saltando`);
