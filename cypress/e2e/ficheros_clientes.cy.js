describe('FICHEROS (CLIENTES) - Validación dinámica desde Excel', () => {
  ...
  const { crearHelpersListadoClientes } = require('../support/clientes/clientes_listado');
  ...
  const helpersListado = crearHelpersListadoClientes({
    UI,
    abrirPanelColumnas,
    guardarPanelColumnas,
    REGEX_TITULO_COLUMNAS,
  ...
  const { exportarExcelVisible, exportarExcelCompleto } = crearHelperExportacionExcel({
    ...
    abrirPantalla: () => UI.abrirPantalla(),
    esperarTabla: () => UI.esperarTabla(),
    obtenerColumnasVisibles: () => obtenerColumnasVisiblesClientes(),
    registrarResultado: registrarResultadoAutomatico,
    validarResultado({ modo, totalFilas, columnasVisibles, cabecerasExcel, normalizarCabecera }) {
  ...
  const { ordenarOtraPagina: ordenarOtraPaginaGlobal } = crearHelperOrdenacionPaginada({
    abrirPantalla: () => UI.abrirPantalla(),
    irAPagina: (numeroPagina) => irAPaginaClientes(numeroPagina),
    ordenarColumna: (nombreColumna) => ordenarColumna(nombreColumna),
    obtenerPaginaActual: () => obtenerPaginaActualClientes(),
    obtenerValoresColumna: () => obtenerValoresColumnaCodigoPaginaActual(),
    registrarResultado: registrarResultadoAutomatico,
    paginaObjetivo: 3,
    nombreColumna: 'Código',
    nombreResultado: 'Ordenar otra página',
  });

  const { comprobarPersistenciaOrdenColumna: comprobarPersistenciaOrdenColumnaGlobal } = crearHelperPersistenciaColumnas({
    abrirPantalla: () => UI.abrirPantalla(),
    esperarTabla: () => UI.esperarTabla(),
    obtenerOrdenCabeceras: () => obtenerOrdenCabecerasClientes(),
    moverColumnaDespues: (origen, destino) => moverColumnaDespues(origen, destino),
    navegarParaPersistencia: () => {
      cy.log('Navegando a otro módulo para comprobar persistencia');
      return cy.navegarAMenu('Ficheros', 'Proveedores').then(() => cy.wait(1000));
    },
    registrarResultado: registrarResultadoAutomatico,
    columnaMover: 'Nombre',
    columnaReferencia: 'Razón Social',
    nombreResultado: 'Cambiar una columna y comprobar que se graba',
    encontrarIndiceColumna: (ordenCabeceras, nombreColumna) =>
      ordenCabeceras.findIndex((txt) => obtenerPatronColumna(nombreColumna).test(txt)),
  });
  ...
  function obtenerFuncionPorNumero(numero) {
    switch (numero) {
      ...
      case 42:
        return { fn: exportarExcelCompleto, autoRegistro: true };
      case 43:
        return { fn: ordenarOtraPaginaGlobal, autoRegistro: false };
      case 44:
