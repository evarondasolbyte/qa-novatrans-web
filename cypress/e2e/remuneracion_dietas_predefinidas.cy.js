describe('REMUNERACIÓN (DIETAS PREDEFINIDAS) - Validación completa con gestión de errores y reporte a Excel', () => {
    ...
    const casos = [
        ...
        { numero: 5, nombre: 'TC005 - Filtrar por ID', funcion: () => ejecutarFiltroIndividual(5), prioridad: 'ALTA' },
        ...
        { numero: 8, nombre: 'TC008 - Filtrar por Concepto (contenga)', funcion: () => ejecutarFiltroIndividual(8), prioridad: 'ALTA' },
        { numero: 9, nombre: 'TC009 - Filtrar por Unidades', funcion: () => ejecutarFiltroIndividual(9), prioridad: 'MEDIA' },
        { numero: 10, nombre: 'TC010 - Filtrar por Precio', funcion: () => ejecutarFiltroIndividual(10), prioridad: 'MEDIA' },
        { numero: 11, nombre: 'TC011 - Filtrar por Total', funcion: () => ejecutarFiltroIndividual(11), prioridad: 'MEDIA' },
        { numero: 12, nombre: 'TC012 - Filtrar por Porcentaje', funcion: () => ejecutarFiltroIndividual(12), prioridad: 'MEDIA' },
        ...
        { numero: 16, nombre: 'TC016 - Búsqueda con espacios al inicio/fin', funcion: () => ejecutarFiltroIndividual(16), prioridad: 'MEDIA' },
    ...
    function ejecutarFiltroIndividual(numeroCaso) {
        ...
        // Obtener datos del Excel para Remuneración-Dietas Predefinidas
        return cy.obtenerDatosExcel('Remuneración-Dietas Predefinidas').then((datosFiltros) => {
            ...
            if (filtroEspecifico.valor_etiqueta_1 === 'columna') {
                ...
                cy.get('select[name="column"], select#column').should('be.visible').then($select => {
                    ...
                    switch(filtroEspecifico.dato_1) {
                        case 'ID':
                            columnaEncontrada = options.find(opt => opt.includes('ID'));
                            break;
                        case 'Conductor':
                        ...
                        case 'Fecha':
                            ...
                            break;
                        case 'Concepto':
                            columnaEncontrada = options.find(opt => opt.includes('Concepto') || opt.includes('Concept'));
                            break;
                        case 'Unidades':
                            columnaEncontrada = options.find(opt => opt.includes('Unidades') || opt.includes('Units'));
                            break;
                        case 'Precio/L':
                            columnaEncontrada = options.find(opt => opt.includes('Precio') || opt.includes('Price') || opt.includes('Precio/L'));
                            break;
                        case 'Total':
                            columnaEncontrada = options.find(opt => opt.includes('Total'));
                            break;
                        case 'Porcentaje':
                            columnaEncontrada = options.find(opt => opt.includes('Porcentaje') || opt.includes('Percentage'));
                            break;
                        default:
                ...
                if (!filtroEspecifico.dato_2 || filtroEspecifico.dato_2.trim() === '') {
                    ...
                    cy.registrarResultados({
                        numero: numeroCaso,
                        nombre: `TC${numeroCasoFormateado} - Filtrar dietas predefinidas por ${filtroEspecifico.dato_1}`,
                ...
                cy.get('body').then($body => {
                    ...
                    if (busquedaSeAplico) {
                        ...
                        cy.registrarResultados({
                            numero: numeroCaso,
                            nombre: `TC${numeroCasoFormateado} - Búsqueda general de dietas predefinidas`,
